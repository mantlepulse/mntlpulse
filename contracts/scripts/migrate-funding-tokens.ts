...
async function main() {
  ...
  console.log("=== Migration: Set Funding Tokens for Existing Polls ===");
  ...
  for (let i = 0; i < Number(nextPollId); i++) {
    try {
      ...
      // Check if funding token is already set
      if (fundingToken !== '0x0000000000000000000000000000000000000000') {
        console.log(`  Poll ${i} already has fundingToken set: ${fundingToken}`);
        console.log(`  Skipping...\n`);
        continue;
      }

      // Parse token from title
      const tokenAddress = parseTokenFromTitle(question, network.name);

      // Set funding token
      console.log(`  Setting fundingToken to: ${tokenAddress}`);
      const tx = await PollsContract.setFundingToken(i, tokenAddress);
      await tx.wait();
